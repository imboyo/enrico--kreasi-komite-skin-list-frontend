...
Retrieve paginated message history for a specific thread, ordered newest-first.
Uses cursor-based pagination via `before` (ISO 8601 datetime of the oldest loaded message).

- **Method:** `POST`
- **URL:** `/admin/skin-chat/thread-messages/:threadId`
- **Status:** `200 OK`
- **Auth:** `AdminGuard` (ADMIN role)
...
```json
POST /admin/skin-chat/thread-messages/xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx
{}
```
...
## 4. Clear Thread Conversation

Delete all messages in a specific thread and reset the thread's `last_message_at` to null.

- **Method:** `DELETE`
- **URL:** `/admin/skin-chat/thread-clean/:threadId`
- **Auth:** `AdminGuard` (ADMIN role)
...
- **Method:** `POST`
- **URL:** `/admin/skin-chat/thread-reply/:threadId`
- **Auth:** `AdminGuard` (ADMIN role)
...
```json
POST /admin/skin-chat/thread-reply/xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx
{
  "message": "Thank you for reaching out! Here's our recommendation..."
}
```
